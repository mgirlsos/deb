...
#
# Info:
#  - Oracle data are gathered real-time
##################################
# Contact:
...
#  20170502	TZ	Add credentials conf
##################################
...
# Oracle Access Access File
ORACLE_ACCESS="$CURRENT_DIR/../conf/oracle_credentials.sql"
...
if [ "$sql" != "" ]; then
       echo "$sql" | $SQLPLUS_BIN -s /nolog @$ORACLE_ACCESS
       rval=$?
fi

if [ "$rval" -ne 0 ]; then
  echo "$ERROR_RUN_COMMAND"
fi
